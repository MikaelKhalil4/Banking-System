...
using NotFoundException = Foxera.Common.CustomExceptions.NotFoundException;

namespace UserAccountService.Application.Helper;
...
public  class AccountHelper
{
    ...
    public async Task<Account> GetAsync(long accountId, CancellationToken cancellationToken)//used for editing an account
    {
        var desiredAccount = await _context.Accounts.SingleOrDefaultAsync(x => x.Id == accountId);
    ...
    public async Task<List<Account>> GetAsync(CancellationToken cancellationToken)//used for get queries
    {
        ...
        return await _context.Accounts.Where(filters).ToListAsync(cancellationToken);
    }
